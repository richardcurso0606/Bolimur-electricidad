...
def renderizar():
    ...
    st.markdown(f"""
    <div style="background-color: #f0f6fc; padding: 16px; border-radius: 8px; border-left: 5px solid #0969da; margin-bottom: 15px;">
    <h4>1. Intensidad de Diseño Trifásica ($I_b$)</h4>
    
    <b>Criterio y Fórmula Reglamentaria:</b>
    $$I_b = \\frac{{P}}{{\\sqrt{{3}} \\cdot V \\cdot \\cos\\varphi}}$$
    
    <b>Leyenda y Definición de Variables:</b><br>
    * <b>I_b</b>: Intensidad de cálculo o de diseño por fase (A).<br>
    * <b>P</b>: Potencia total prevista de transporte en la línea ($P_t$ = {lga_pot:,.2f} W).<br>
    * <b>V</b>: Tensión nominal compuesta entre fases (400 V).<br>
    * <b>cos φ</b>: Factor de potencia estimado para instalaciones generales (0.9).<br>
    * <b>√3</b>: Constante trifásica ($\\approx 1.732$).
    
    <br><b>Sustitución Numérica y Resultado:</b>
    $$I_b = \\frac{{{lga_pot:,.2f} \\text{{ W}}}}{{\\sqrt{{3}} \\cdot 400 \\text{{ V}} \\cdot 0.9}} = \\frac{{{lga_pot:,.2f}}}{{623.54}} = \\mathbf{{ {ib_lga:.2f} \\text{{ A}} }}$$
    </div>
    """, unsafe_allow_html=True)

    # --- BLOQUE 2: SECCIÓN POR CAÍDA DE TENSIÓN (CORREGIDO Y ESTABLE) ---
    st.markdown(f"""
    <div style="background-color: #f0f6fc; padding: 16px; border-radius: 8px; border-left: 5px solid #0969da; margin-bottom: 15px;">
    ...
    <b>Leyenda y Definición de Variables:</b><br>
    * <b>S</b>: Sección teórica mínima exigida del conductor ($\\text{{mm}}^2$).<br>
    * <b>P</b>: Potencia total de cálculo ({lga_pot:,.2f} W).<br>
    * <b>L</b>: Longitud unifilar de la línea ({lga_long} m).<br>
    * <b>γ</b>: Conductividad del material a servicio normal ({gamma_lga} m/(Ω·mm²) para {lga_aisl}).<br>
    * <b>ΔV</b>: Caída de tensión máxima admisible ({dv_pct_lga}% de 400V = {dv_max_lga:.2f} V).<br>
    * <b>V</b>: Tensión nominal (400 V).
    ...
    <b>Criterio y Fórmula Reglamentaria (ITC-BT-14 / ITC-BT-22):</b><br>
    ...
    <b>Leyenda y Definición de Variables:</b><br>
    * <b>I_cc,final</b>: Corriente de cortocircuito estimada al final de la LGA (A).<br>
    * <b>V</b>: Tensión nominal compuesta de referencia (400 V).<br>
    * <b>I_cc,origen</b>: Corriente de cortocircuito en el origen de la línea ({lga_icc_orig * 1000:,.0f} A).<br>
    * <b>R_cable</b>: Resistencia activa del conductor en el tramo ($R = \\frac{{\\rho \\cdot L}}{{S}} = {r_lga_cable:.5f}\\ \\Omega$).<br>
    * <b>In</b>: Calibre de los fusibles gG de protección en origen ({in_lga_auto} A).
    
    <br><b>Sustitución Numérica y Verificación Reglamentaria:</b>
    ...
    * <b>Icc al final de la LGA:</b> <b>{icc_fin_lga:.1f} A</b> ({icc_fin_lga / 1000.0:.2f} kA)<br>
    * <b>Veredicto de Coordinación:</b> ✅ La corriente de cortocircuito al final de la línea garantiza la fusión de los fusibles de protección de <b>{in_lga_auto} A (Tipo gG)</b> dentro de los márgenes reglamentarios exigidos por el REBT.
    </div>
    """, unsafe_allow_html=True)
