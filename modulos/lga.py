...
def renderizar():
    ...
    st.info(f"""
    #### 1. Intensidad de Diseño Trifásica ($I_b$) — Explicación para Principiantes
    
    * **¿Qué calculamos aquí?** Averiguamos cuántos amperios (fuerza de corriente) van a circular por los cables del edificio.
    * **Fórmula:** $I_b = \\frac{{P}}{{\\sqrt{{3}} \\cdot V \\cdot \\cos\\varphi}}$
    * **Traducción de variables:**
      * **P**: Potencia total del edificio ({lga_pot:,.2f} W sumando viviendas, locales, servicios y garajes).
      * **$\sqrt{{3}} \\cdot V \\cdot \\cos\\varphi$**: Constantes trifásicas de España (1.732 $\\cdot$ 400 V $\\cdot$ 0.9 de eficiencia = 623.54).
    * **Resultado numérico:** $\\frac{{{lga_pot:,.2f}}}{{623.54}} = \\mathbf{{{ib_lga:.2f} \\text{{ A}}}}$
    """)

    # --- BLOQUE 2: SECCIÓN POR CAÍDA DE TENSIÓN ---
    st.info(f"""
    #### 2. Sección Teórica del Cable — Explicación para Principiantes
    
    * **¿Qué calculamos aquí?** Qué grosor de cobre necesitamos para que la luz llegue a los contadores sin perder fuerza por el camino (respetando el límite de caída del {dv_pct_lga}%).
    * **Fórmula:** $S = \\frac{{P \\cdot L}}{{\\gamma \\cdot \\Delta V \\cdot V}}$
    * **Traducción de variables:**
      * **P** ({lga_pot:,.2f} W) y **L** ({lga_long} m de distancia de cable).
      * **$\gamma$** ({gamma_lga} conductividad del material) y **$\Delta V$** ({dv_max_lga:.2f} V de pérdida máxima permitida).
    * **Resultado y Selección Comercial:** El cálculo da una base teórica que se redondea automáticamente al siguiente tamaño real que se vende en el almacén: **{s_final_lga} mm²**.
    """)
    
    # --- BLOQUE 3: ICC MÍNIMA Y FUSIBLES ---
    st.info(f"""
    #### 3. Icc Mínima y Fusibles de Compañía (CGP) — Explicación para Principiantes
    
    * **¿Qué comprobamos aquí?** Que si hay un cortocircuito al final de la línea, los fusibles de seguridad de la caja general (CGP) salten rápido para proteger la instalación.
    * **Resultado de Icc final:** **{icc_fin_lga:.1f} A** ({icc_fin_lga / 1000.0:.2f} kA).
    * **Veredicto:** ✅ Garantiza la fusión correcta de los elementos de protección.
    """)
